...
static void
ngx_event_resolver_test_result(void *d, struct sockaddr *sa, socklen_t socklen)
{
    ngx_chain_t                     cl;
    ngx_buf_t                      *b;
    size_t                          len;
    ...
    len = ngx_sock_ntop(sa, socklen, text, NGX_SOCKADDRLEN, 0) + 1;
...
ngx_event_resolver_test_handler(ngx_http_request_t *r)
{
    ...
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "after start resolver");
