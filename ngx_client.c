...
static void ngx_client_connect_server(void *data, struct sockaddr *sa,
        socklen_t socklen);
...
ngx_client_reconnect(ngx_client_session_t *s)
{
    ...
    ++s->peer.tries;
    ngx_add_timer(&s->connect_event, s->ci->timeout);
...
closed:
    ...
    ngx_log_error(NGX_LOG_INFO, c->log, err, "nginx client connection error");
...
ngx_int_t
ngx_client_write(ngx_client_session_t *s, ngx_chain_t *in)
{
    ...
    *ll = NULL;
    ...
    if (!last && !flush && in && size < (off_t) s->ci->postpone_output) {
        return NGX_OK;
    }
...
ngx_chain_t *
ngx_client_read(ngx_client_session_t *s)
{
    return NULL;
}
...
ngx_client_handler(ngx_event_t *ev)
{
    ...
    if (ev->write) {
        ...
        if (ngx_client_test_reading(s) == NGX_OK) {
            ...
            ngx_log_error(NGX_LOG_INFO, s->log, 0,
                    "nginx client handler recv error");
            if (s->ci->max_retries && s->peer.tries > s->ci->max_retries) {
                ...
                ngx_log_error(NGX_LOG_ERR, s->log, 0,
                        "nginx client handler reconnect");
...
ngx_client_create_session(ngx_client_init_t *ci)
{
    ...
    if (ci->timeout == 0) { /* set default reconnect timeout */
        ci->timeout = 3000;
    }
